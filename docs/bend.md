...
- **A record's width is a budget the whole program shares, and 255 is the cap.** `bend f.bend -o out` dies with "an arity over 255" where the JS lane builds the same program: a segment's parameters are every value live at that point, and a record counts as its leaves. With milestone 6's sim a sixteenth field on the player builds if it is a `U32` and fails if it is a record (which costs its width) or if three more are added anywhere, the inventory included. `Sim.turn` is where the budget goes: it holds the state it began with, the one its thinker made and the one it hands back, 78 words each beside the geometry's 24, so each word on the player costs three. Put counts that will not fit in a `V.Vec`, as the powers and `Sim.Hurt` do; take what a helper reads rather than the record it reads it from (`State.held` takes the place, not the state); or derive the value (the muzzle flash's extra light is its sprite's row, not a field). A patched copy of the compiler's `comp.ts` names the offending segment, which is the only way to find it.
