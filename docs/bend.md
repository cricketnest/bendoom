...
- **A def's live values are its native arity, and 255 is the cap.** `bend f.bend -o out` dies with "an arity over 255" where the JS lane builds the same program: a segment's parameters are every value live at that point, and a record counts as its leaves, so a big record held three times over blows the byte the runtime keeps the arity in. `Sim.turn` holds the state it began with, the one its thinker made and the one it hands back, 78 words each beside the geometry's 24; one more word on the player costs three. Group new player fields into a record only if that removes leaves (it does not), or take what a helper reads rather than the record it reads it from, or derive the value (the muzzle flash's extra light is its sprite's row, not a field). A patched copy of the compiler's `comp.ts` names the offending segment, which is the only way to find it.
