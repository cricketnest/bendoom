...
- The table is `Tables.rndtable`, generated with the others: `tools/gen_tables.nu` now takes `tables.c` and `m_random.c`, and the other three tables came out byte for byte as they were. `Random.at` in `src/random.bend` (`Sim.random` until milestone 5) reads the list at the index; until milestone 5's ticket 12 it built a tree from the table on every draw. A draw happens on a flash's turn, a few times a second, so the table is not a field of the geometry, which would have put it in every literal level of the laws.
