...
- `Thing.Row` carries sprite, frame, full-bright bit, tics and next row. The rows are the 59 reachable states in info.c's order, and the definitions point at their spawn rows among them. A lasting row's next, vanilla's S_NULL, names the row itself. Nothing takes it.
...
- The sprite table has a fifth word a row, 1 for a full-bright frame. Projection then gives such a frame the first colormap and every other frame its sector-light colormap, with no row lookup per frame.
