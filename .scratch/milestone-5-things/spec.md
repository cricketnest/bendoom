...
- Position checking examines things before lines, as vanilla does. A solid thing blocks when the two radii overlap at intersecting heights. A special thing touched by the player tries its grant and is removed only on success. The pure check returns the changed population and player inventory with its fit result. If vanilla can consume a pickup during a position check whose later line check rejects the move, Bendoom preserves that order.
- Moving sectors re-clip the player and active things in the sector's block box through the same height rules. A thing standing on the old floor follows the new floor. Crushing damage and barrel explosions remain inert, but a solid thing that no longer fits makes the mover report the same blocked result as an undamageable vanilla thing.
