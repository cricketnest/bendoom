...
Checks: the proof (`All terms check.`); the affected tests (load, ranges, route, sim, game, render) on both lanes; 40 oracle cases on Freedoom and 3 on the shareware WAD; `nix flake check`; `git diff --check`; `doom.bend`, `film.bend`, `tools/walk.bend` and the benches build.
