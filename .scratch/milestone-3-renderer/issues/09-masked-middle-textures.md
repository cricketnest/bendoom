...
Addendum from the code review (ticket 12): `Segs.sil` let a masked texture overwrite the silhouette heights, where vanilla fills in only a way nothing else clips (a masked range whose floor steps down keeps the front's floor as the height it clips under). Nothing reads the heights until milestone 5's sprites, so no frame differed; it is fixed, and the closed law `silhouettes` pins a one-sided wall, that masked case, a plain window and a shut door.
