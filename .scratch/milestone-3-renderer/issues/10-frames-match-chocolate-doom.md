...
| place | differing | varying | pistol |
| --- | --- | --- | --- |
| -416 256 0 (start) | 0 | 935 | 1793 |
| 736 464 135 (lit) | 0 | 0 | 1793 |
| -416 256 90 (dark) | 0 | 0 | 1793 |
| 137 256 45 (step) | 0 | 159 | 1793 |
| -416 256 180 (grate, sky behind) | 0 | 0 | 1793 |
| 3 256 0 (water ceiling) | 0 | 4440 | 1793 |
| -640 256 90 (open sky) | 0 | 0 | 1793 |
| 488 256 0 (pit) | 0 | 1429 | 1793 |

Chocolate Doom 3.1.1 from nixpkgs on Freedoom 0.13.0, of 53760 view pixels each. A pixel of ours passes when it is that pixel in any of Chocolate Doom's eight shots, so "differing" is zero for the animated surfaces too: every pixel of the water ceiling and the computer screens, which Bendoom draws in their first frame and does not animate yet (animation is not in this milestone's stories), is the pixel of one of vanilla's frames. "varying" counts the pixels that change among the shots. The first version of the tool dropped the varying pixels from the count instead of checking them; the code review caught that, and the table is from the stricter tool.
...
Story 26, the shareware WAD: `doom1.wad` loads and renders. Its start (`FRAME="1056 -3616 90"` through `tools/frame.bend`) is id's E1M1: the hexagon floor, the pillars, the blue carpet, every pixel of the view written. The oracle cannot run on it: Chocolate Doom, like vanilla, refuses a PWAD with the shareware IWAD, and the oracle's single player start is a PWAD. So the zero-difference claim is measured on Freedoom only, through the same code.
