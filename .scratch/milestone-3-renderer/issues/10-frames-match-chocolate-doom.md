...
- Chocolate Doom from nixpkgs does not start natively on Wayland here (with `SDL_VIDEODRIVER=wayland` it only puts up an error dialog), so it runs under Xwayland and takes its key from xdotool; the first key press after niri focuses it is swallowed, so the tool presses until the shots exist.
