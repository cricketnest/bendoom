...
random tables. A Chocolate Doom GDB trace records `sawup` at tic 50,
`sawidl` from tic 65, repeated `sawful` while held against a target,
and the return to `sawidl`. The sim cases record fireball flight starts
and the stop on removal before `firxpl`.
...
Chocolate Doom's Freedoom trace places five pistol restarts at frames
114688, 131072, 147456, 163840, and 179200, a barrel explosion at
148480, and the barrel removal stop at 183296. A native Bend fixture
imports the current `Stream`, `Channel`, and `Sound` modules. It renders
114688 frames, starts the pistol, renders 16384, restarts it, renders
16384, restarts it, renders 1024, starts the barrel, renders 15360,
restarts the pistol, renders 15360, restarts it, renders 4096, stops
`Thing{3}`, and renders the last 89088 frames. The current Freedoom
sounds and isolated OPL stream produce 272384 frames. Running
`tools/listen.nu compare` on that raw output and Chocolate Doom's
capture reports `frames 228327`, `differing 0`, `largest 0` after lead
alignment.
